...
{ 
  ...
  ((hsram->Instance)->BTCR[(hsram->Init . NSBank)] |= (0x1U << (0U))); 
...
{ 
  ...
  do{ (hsram)->Lock = HAL_UNLOCKED; }while (0U);
...
HAL_StatusTypeDef HAL_SRAM_Read_8b(SRAM_HandleTypeDef *hsram, uint32_t *pAddress, uint8_t *pDstBuffer, uint32_t BufferSize)
{
  ...
  do{ if((hsram)->Lock == HAL_LOCKED) { return HAL_BUSY; } else { (hsram)->Lock = HAL_LOCKED; } }while (0U);
