...
{
	uint16_t Width, Height, column;
	Width = (128 % 8 == 0)? (128 / 8 ): (128 / 8 + 1);
	Height = 64;  
	OLED_WriteReg(0xb0); 	
...
{		
	...
	uint16_t Width, Height, column, temp;
	Width = (128 % 8 == 0)? (128 / 8 ): (128 / 8 + 1);
	Height = 64;   
	...
	for (uint16_t j = 0; j < Height; j++) {
        ...
        if(HAL_I2C_Master_Transmit(&hi2c1, (0X3C << 1) | 0X00, W_Buf, sLen, 2) != HAL_OK) {
            Err++;
