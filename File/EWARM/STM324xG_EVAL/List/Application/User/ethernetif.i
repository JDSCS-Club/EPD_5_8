...
u32_t sys_now(void);
 ...
 ETH_DMADescTypeDef  DMARxDscrTab[(4U)] ; 

  #pragma data_alignment=4   
 ETH_DMADescTypeDef  DMATxDscrTab[(4U)] ; 

  #pragma data_alignment=4   
 uint8_t Rx_Buff[(4U)][1524U] ;  

  #pragma data_alignment=4   
 uint8_t Tx_Buff[(4U)][1524U] ;  
...
{ 
  ...
  HAL_ETH_WritePHYRegister(&EthHandle, ((uint16_t)0x12), regvalue);
}
...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != 0; q = q->next)
  {
    ...
    if((DmaTxDesc->Status & 0x80000000U) != (uint32_t)RESET)
    {
      errval = ERR_USE;
      goto error;
    }
    ...
    while( (byteslefttocopy + bufferoffset) > 1524U )
    {
       
      memcpy( (uint8_t*)((uint8_t*)buffer + bufferoffset), (uint8_t*)((uint8_t*)q->payload + payloadoffset), (1524U - bufferoffset) );
      ...
      byteslefttocopy = byteslefttocopy - (1524U - bufferoffset);
      payloadoffset = payloadoffset + (1524U - bufferoffset);
      framelength = framelength + (1524U - bufferoffset);
      bufferoffset = 0;
    }
...
error:
  ...
  if ((EthHandle.Instance->DMASR & (0x1U << (5U))) != (uint32_t)RESET)
  {
     
    EthHandle.Instance->DMASR = (0x1U << (5U));
...
static struct pbuf * low_level_input(struct netif *netif)
{
  ...
  if (p != 0)
  {
    ...
    for(q = p; q != 0; q = q->next)
    {
      ...
      while( (byteslefttocopy + bufferoffset) > 1524U )
      {
        ...
        memcpy( (uint8_t*)((uint8_t*)q->payload + payloadoffset), (uint8_t*)((uint8_t*)buffer + bufferoffset), (1524U - bufferoffset));
  ...
  if ((EthHandle.Instance->DMASR & (0x1U << (7U))) != (uint32_t)RESET)  
  {
     
    EthHandle.Instance->DMASR = (0x1U << (7U));
...
void ethernetif_set_link(struct netif *netif)
{
  ...
  if((regvalue & ((uint16_t)0x2000U)) != (uint16_t)RESET)
  {
     
    HAL_ETH_ReadPHYRegister(&EthHandle, ((uint16_t)0x10), &regvalue);
    
     
    if((regvalue & ((uint16_t)0x0001))!= (uint16_t)RESET)
    {
...
void ethernetif_update_config(struct netif *netif)
{
  ...
  { 
    ...
    if(EthHandle.Init.AutoNegotiation != 0x00000000U)
    {
       
      HAL_ETH_WritePHYRegister(&EthHandle, ((uint16_t)0x00), ((uint16_t)0x1000));
      ...
      do
      {
        HAL_ETH_ReadPHYRegister(&EthHandle, ((uint16_t)0x01), &regvalue);
      ...
      } while (((regvalue & ((uint16_t)0x0020)) != ((uint16_t)0x0020)));
      ...
      {  
        ...
        EthHandle.Init.Speed = 0x00000000U; 
      }
      else
      {   
    ...
    error :
      ...
      HAL_ETH_WritePHYRegister(&EthHandle, ((uint16_t)0x00), ((uint16_t)(EthHandle.Init.DuplexMode >> 3) |
                                                     (uint16_t)(EthHandle.Init.Speed >> 1))); 
    }

     
    HAL_ETH_ConfigMAC(&EthHandle, (ETH_MACInitTypeDef *) 0);
