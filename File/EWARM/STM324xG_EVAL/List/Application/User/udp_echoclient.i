...
volatile   uint8_t  Syslog_Flag = 0;

volatile uint16_t mUdp_Server_Port = 0x600;
...
void udp_echoclient_connect(void)
{
    ...
    if (upcb!=0) 
    {
        ...
        err= udp_connect(upcb, &mLed_Process_Flag.sDestIPaddr, 30013);

        if (err == ERR_OK) 
        {
             
            udp_recv(upcb, udp_receive_callback, 0);
...
void udp_SysLog_Connect(int Kind_Code,char * format, ... )
{
    ...
    else 
    {
        ...
        if (upcb!=0) 
        {
            ...
            (&DestIPaddr)->addr = ((u32_t)((255) & 0xff) << 24) | ((u32_t)((255) & 0xff) << 16) | ((u32_t)((255) & 0xff) << 8) | (u32_t)((255) & 0xff);
            ...
            if(Kind_Code)  
            {
                err= udp_connect(upcb, &DestIPaddr, 3338); 
            }
            else 
            {
...
void udp_SysLog(char * format, ... )
{
    ...
    else 
    {
        ...
        if (upcb!=0) 
        {
            ...
            (&DestIPaddr)->addr = ((u32_t)((100) & 0xff) << 24) | ((u32_t)((100) & 0xff) << 16) | ((u32_t)((128) & 0xff) << 8) | (u32_t)((10) & 0xff);
