...
_Pragma("function_effects = no_state, always_returns") __intrinsic __nounwind int    isascii(int);
...

{
	...
	for ( argc = 0; argc < 20 && p != 0; p = (char *)strtok_r( 0, sepchr, &saveptr ) )
	{
...

{
	...
	if ( argc != 0 )
	{
		...
		for ( i = 0; i < (sizeof(user_command_table) / sizeof((user_command_table)[0])); i++ )
		{
...

{
	...
	for ( ;; )
	{
		...
		else if ( (0x20 <= ch) && (ch <= 0x7E) )
		{
			...
			if ( command_index < (64 - 2) )
			{
				command_buf[command_index++] = ch;
			}
			MyPrintf_USART1( "%c", ch );    
...

{
	...
	for ( i = 1; i < (sizeof(user_command_table) / sizeof((user_command_table)[0])); i++ )
	{
		if ( 1 < argc )
		{
