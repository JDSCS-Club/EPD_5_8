...
uint8_t Tcp_flag = 0;
uint8_t mTCP_Tx_Buf[100];
uint8_t mTCP_Tx_message_count = 0;
...
void tcp_echoserver_init(void)
{
    ...
    if (tcp_echoserver_pcb != 0)
    {
        err_t err;

         
        err = tcp_bind(tcp_echoserver_pcb, (&ip_addr_any),10900);

        if (err == ERR_OK)
        {
             
            tcp_echoserver_pcb = tcp_listen_with_backlog(tcp_echoserver_pcb, 0xff);
...
static err_t tcp_echoserver_accept(void *arg, struct tcp_pcb *newpcb, err_t err)
{
    ...
    if (es != 0)
    {
        ...
        MyPrintf_USART1 (" tcp_echoserver_accept  \n\r");
    }
    else
    {
...
uint8_t d_Ethernet_TCP_TestBuf[100];
...
static void tcp_echoserver_connection_close(struct tcp_pcb *tpcb, struct tcp_echoserver_struct *es)
{
    ...
    MyPrintf_USART1 (" tcp_echoserver_connection_close  \n\r");
}
