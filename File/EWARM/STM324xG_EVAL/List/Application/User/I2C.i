...
void processCurrentVal(void)
{
    ...
    if(bCurLedCtr) 
    {
         ...
         if(I2C_HAL_ReadBytes(&hi2c1,0x80,0x04,(uint8_t *)nRbuf,2))
        {
             ...
             if(mLed_Process_Flag.sCurrentVal < sData_val) 
             {
	...
	if (R_s_SpkCheckCnt)
	{
        ...
        if (getSW_RS() || getSW_AR()) 
		{
            ...
            MyPrintf_USART1 ("+++++ AMP T-1 : %d \r\n",HAL_GetTick() );
                ...
                {
                    if ((mAnsSetFlag.tAnsCnt > 15) || (mLed_Process_Flag.tAmp_Vol_UpFlag == 1))
                    {
                        mAnsSetFlag.tAnsFlgSet = 1;
                        njw1192_vol_setting(mLed_Process_Flag.tAmp_Vol_UpFlag, 1); 
                    }
                    else
                    {
                        if (mAnsSetFlag.tAnsFlgSet) 
                        {
                            mAnsSetFlag.tAnsFlgSet = 0;
                ...
                if(s_SpCh1 == 1 && s_SpCh2 == 0)
                {
                    ...
                    else if(mLed_Process_Flag.sCurrentTestFlag == 1) 
                    {
                        AMP_Init(AMP_ID_1);
		...
		else if ((getSW_SL() && (mLed_Process_Flag.sDHCP_IP_Val == 96)) 
               ||(getSW_SR() && (mLed_Process_Flag.sDHCP_IP_Val == 97)))
		{
            ...
            mLed_Process_Flag.sRom_Spk_Flag = 0;
            mLed_Process_Flag.sOut_Spk_Flag = 1;
            mLed_Process_Flag.sSpk_check_Cnt = 10;
            mLed_Process_Flag.sAudio_Play_mode = 1;
		...
		else
		{
            ...
            mLed_Process_Flag.sAudio_Play_mode = 0;
...

{
	...
	TimeOut = 0;
	while (HAL_I2C_Mem_Write(hi2c, (uint16_t)DevAddress, (uint16_t)MemAddress, 0x00000001U, pData, (uint16_t)TxBufferSize,2) != HAL_OK && TimeOut < 2) TimeOut++;
...

{
	...
	TimeOut = 0;
	while (HAL_I2C_Mem_Read(hi2c, (uint16_t)DevAddress, (uint16_t)MemAddress, 0x00000001U, pData, (uint16_t)RxBufferSize, 2) != HAL_OK && TimeOut < 2) TimeOut++;
...
void AMP_FAULT(void)
{
    ...
    else
    {
        ...
        if(sAmpRestCnt) 
        {
            ...
            if(mLed_Process_Flag.sAmp_Fault_Cnt_All >5 ) 
            {
...
void AMP_SPK_CHECK(void)
{
    ...
    if(mLed_Process_Flag.sRom_Spk_Flag) 
    {
    ...
    else if(mLed_Process_Flag.sOut_Spk_Flag) 
     {
         mLed_Process_Flag.sOut_Spk_Flag = 0;
