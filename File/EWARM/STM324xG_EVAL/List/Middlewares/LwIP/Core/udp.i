...
void FLASH_Read_Len(volatile uint32_t* FlashAddress,uint8_t* Data ,uint16_t DataLength);
extern volatile uint32_t BootFlashReadAddress;
...
udp_input(struct pbuf *p, struct netif *inp)
{
  ...
  uint64_t sRxBitCompare = 0; 

  
  sUDP_Rx_PointBuf = (uint8_t *)0x20017000; 
  ...
  for (pcb = udp_pcbs; pcb != 0; pcb = pcb->next) {
    ...
    if(5202 == lwip_htons(udphdr->dest))
    {
      
        memcpy(mLed_Process_Flag.sRx_Public_Buf,(uint8_t*)((uint8_t*)p->payload +(8)),sizeof(mETHERNET_RX_STRUCT));
               
        pmEthernet_Rx_struct = (mETHERNET_RX_STRUCT *)mLed_Process_Flag.sRx_Public_Buf;

        
        memcpy(sIpAddBuf,p,100); 
        iphdr = (struct ip_hdr *)&sIpAddBuf[30] ; 
  
        if( pmEthernet_Rx_struct->sSDR_TYPE_00 == 0x30) 
        {
            mLed_Process_Flag.sEth_Rx_Cnt= 0;
                ...
                if( pmEthernet_Rx_struct->sSDR_Command_03 == 0x11) 
                {
                    
                     
                    
                    mLed_Process_Flag.sEth_Rx_Finish_TimeCnt = 1000; 
                    ...
                    (&mLed_Process_Flag . sDestIPaddr)->addr = ((u32_t)(((((const u8_t*)(&(&iphdr->src)->addr))[3])) & 0xff) << 24) | ((u32_t)(((((const u8_t*)(&(&iphdr->src)->addr))[2])) & 0xff) << 16) | ((u32_t)(((((const u8_t*)(&(&iphdr->src)->addr))[1])) & 0xff) << 8) | (u32_t)(((((const u8_t*)(&(&iphdr->src)->addr))[0])) & 0xff);
                }
            
        }
    }
    else if(30000 == lwip_htons(udphdr->dest))
    {
        mLed_Process_Flag.sEth_Rx_Cnt = 0;
        
        memcpy(mLed_Process_Flag.sRx_Public_Buf,(uint8_t*)((uint8_t*)p->payload +(8)),50);
        
         if(lwip_htons(udphdr->len) < 30)
        {
            ...
            {
                mLed_Process_Flag.sAnsm_Run_Flag = 0;
            ...
            else if (strncmp((char*)(mLed_Process_Flag.sRx_Public_Buf), "AMP_VOL_UP", 10) == 0)
            {
                ...
                sTrainPoint = ((((mLed_Process_Flag . sRx_Public_Buf[11]&0x0F) << 4) | ((mLed_Process_Flag . sRx_Public_Buf[12]&0x0F) & 0x0f)) & 0xff);
                if( sTrainPoint == (mLed_Process_Flag.sTrainID))
                {
                    mLed_Process_Flag.tAmp_Vol_UpFlag = 1;
                    
                    if(mLed_Process_Flag.sVolTestFlg == 1) 
                    {
            ...
            else if (strncmp((char*)(mLed_Process_Flag.sRx_Public_Buf), "AMP_VOL_NO", 10) == 0)
            {
                ...
                if(sTrainPoint == (mLed_Process_Flag.sTrainID))
                {
                    mLed_Process_Flag.tAmp_Vol_UpFlag = 0;
            ...
            else if (strncmp((char*)(mLed_Process_Flag.sRx_Public_Buf), "TEST_ON", 7) == 0)
            {
                 ...
                 mLed_Process_Flag.sCurrentTestFlag = 1;
                 
                 mLed_Process_Flag.sCurrentVal = 0; 
            }
            else if (strncmp((char*)(mLed_Process_Flag.sRx_Public_Buf), "TEST_OFF", 8) == 0)
            {
                 ...
                 mLed_Process_Flag.sCurrentTestFlag = 0;
			...
			else if (strncmp((char*)(mLed_Process_Flag.sRx_Public_Buf), "Clock", 5) == 0)
			{
				...
				{
					mLed_Process_Flag.sClock_Start_Flage = 1; 

					mLed_Process_Flag.sClock_Data.sHours = (((ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[12]) << 4) | (ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[13]) & 0x0f)) & 0xff);
					mLed_Process_Flag.sClock_Data.sMinutes = (((ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[15]) << 4) | (ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[16]) & 0x0f)) & 0xff);
					mLed_Process_Flag.sClock_Data.sSeconds = (((ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[18]) << 4) | (ConvAsc2Hex(mLed_Process_Flag . sRx_Public_Buf[19]) & 0x0f)) & 0xff);
  ...
  if (for_us) {
    ...
    if (pcb != 0) {
      ...
      if (!broadcast && !((((&ip_data . current_iphdr_dest))->addr & ((((0xf0000000UL) & 0xff) << 24) | (((0xf0000000UL) & 0xff00) << 8) | (((0xf0000000UL) & 0xff0000UL) >> 8) | (((0xf0000000UL) & 0xff000000UL) >> 24))) == ((((0xe0000000UL) & 0xff) << 24) | (((0xe0000000UL) & 0xff00) << 8) | (((0xe0000000UL) & 0xff0000UL) >> 8) | (((0xe0000000UL) & 0xff000000UL) >> 24)))) {
  ...
  const ip_addr_t *dst_ip, u16_t dst_port, struct netif *netif)
{
  ...
  if ((((&pcb->local_ip)) == 0 || ((*((&pcb->local_ip))). addr == ((u32_t)0x00000000UL))) ||
      ((((&pcb->local_ip))->addr & ((((0xf0000000UL) & 0xff) << 24) | (((0xf0000000UL) & 0xff00) << 8) | (((0xf0000000UL) & 0xff0000UL) >> 8) | (((0xf0000000UL) & 0xff000000UL) >> 24))) == ((((0xe0000000UL) & 0xff) << 24) | (((0xe0000000UL) & 0xff00) << 8) | (((0xe0000000UL) & 0xff0000UL) >> 8) | (((0xe0000000UL) & 0xff000000UL) >> 24)))) {
