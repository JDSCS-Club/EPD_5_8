...
tcp_close(struct tcp_pcb *pcb)
{
	...
	MyPrintf_USART1("tcp_close: closing in ");
...
tcp_abandon(struct tcp_pcb *pcb, int reset)
{
  ...
  if (pcb->state == TIME_WAIT) {
    ...
    ;
    if (send_rst) {
	    MyPrintf_USART1("tcp_abandon: sending RST\n");
      tcp_rst(seqno, ackno, &pcb->local_ip, &pcb->remote_ip, local_port, pcb->remote_port);
    }
    memp_free(MEMP_TCP_PCB, pcb);
    do { if((errf) != 0) (errf)((errf_arg),(ERR_ABRT)); } while (0);
  }
}
...
tcp_slowtmr_start:
  ...
  pcb = tcp_active_pcbs;
  if (pcb == 0) {
	  MyPrintf_USART1("tcp_slowtmr: no active pcbs\n");
  }
  while (pcb != 0) {
	  MyPrintf_USART1("tcp_slowtmr: processing active pcb\n");
    ...
    if (pcb->state == SYN_SENT && pcb->nrtx == 6) {
      ++pcb_remove;
	    MyPrintf_USART1("tcp_slowtmr: max SYN retries reached\n");
    }
    else if (pcb->nrtx == 12) {
      ++pcb_remove;
	    MyPrintf_USART1("tcp_slowtmr: max DATA retries reached\n");
    } else {
    ...
    if (pcb->state == FIN_WAIT_2) {
      ...
      if (pcb->flags & 0x10U) {
        ...
        if ((u32_t)(tcp_ticks - pcb->tmr) >
            20000 / (2*250)) {
          ++pcb_remove;
	            MyPrintf_USART1("tcp_slowtmr: removing pcb stuck in FIN-WAIT-2\n");
        }
      }
    }
    ...
    if (((pcb)->so_options & (0x08U)) &&
      ...
      {
	      MyPrintf_USART1("tcp_slowtmr: KEEPALIVE timeout. Aborting connection to ");
        ;
	      MyPrintf_USART1("\n");
    ...
    if (pcb->state == SYN_RCVD) {
      if ((u32_t)(tcp_ticks - pcb->tmr) >
          20000 / (2*250)) {
        ++pcb_remove;
	          MyPrintf_USART1("tcp_slowtmr: removing pcb stuck in SYN-RCVD\n");
      }
    }
    ...
    if (pcb->state == LAST_ACK) {
      if ((u32_t)(tcp_ticks - pcb->tmr) > 2 * 60000UL / (2*250)) {
        ++pcb_remove;
	      MyPrintf_USART1("tcp_slowtmr: removing pcb stuck in LAST-ACK\n");
      }
    }
    ...
    if (pcb_remove) {
      ...
      do { if((err_fn) != 0) (err_fn)((err_arg),(ERR_ABRT)); } while (0);
      ...
      if (prev->polltmr >= prev->pollinterval) {
        prev->polltmr = 0;
	      MyPrintf_USART1("tcp_slowtmr: polling application\n");
        tcp_active_pcbs_changed = 0;
...
tcp_fasttmr_start:
  ...
  while (pcb != 0) {
    if (pcb->last_timer != tcp_timer_ctr) {
      ...
      if (pcb->flags & 0x01U) {
	      MyPrintf_USART1("tcp_fasttmr: delayed ACK\n");
        do { (pcb)->flags |= 0x02U; } while (0);
        tcp_output(pcb);
...
{
  {
    ...
    pcb->refused_data = 0;
	  ...
	  MyPrintf_USART1("tcp_input: notify kept packet\n");
    ...
    if (err == ERR_OK) {
      ...
      if (refused_flags & 0x20U
	    ...
	    MyPrintf_USART1("tcp_input: drop incoming packets, because pcb is \"full\"\n");
      return ERR_ABRT;
    } else {
...
tcp_kill_state(enum tcp_state state)
{
  ...
  do { if (!((state == CLOSING) || (state == LAST_ACK))) { ; }} while(0);
  ...
  if (inactive != 0) {
	  MyPrintf_USART1("tcp_kill_closing: killing oldest\n");
...
tcp_kill_timewait(void)
{
  ...
  if (inactive != 0) {
	  MyPrintf_USART1("tcp_kill_timewait: killing oldest TIME-WAIT PCB\n");
    tcp_abort(inactive);
  }
}
...
tcp_alloc(u8_t prio)
{
  ...
  if (pcb == 0) {
	  ...
	  MyPrintf_USART1("tcp_alloc: killing off oldest TIME-WAIT connection\n");
    tcp_kill_timewait();
     
    pcb = (struct tcp_pcb *)memp_malloc(MEMP_TCP_PCB);
    if (pcb == 0) {
       
	    MyPrintf_USART1("tcp_alloc: killing off oldest LAST-ACK connection\n");
      tcp_kill_state(LAST_ACK);
       
      pcb = (struct tcp_pcb *)memp_malloc(MEMP_TCP_PCB);
      if (pcb == 0) {
         
	      MyPrintf_USART1("tcp_alloc: killing off oldest CLOSING connection\n");
        tcp_kill_state(CLOSING);
         
        pcb = (struct tcp_pcb *)memp_malloc(MEMP_TCP_PCB);
        if (pcb == 0) {
           
	        MyPrintf_USART1("tcp_alloc: killing connection with prio lower than %d\n", prio);
          tcp_kill_prio(prio);
...
tcp_pcb_purge(struct tcp_pcb *pcb)
{
  if (pcb->state != CLOSED &&
     ...
     pcb->state != LISTEN) {
	     ...
	     MyPrintf_USART1("tcp_pcb_purge\n");

    ;

    if (pcb->refused_data != 0) {
	    MyPrintf_USART1("tcp_pcb_purge: data left on ->refused_data\n");
      pbuf_free(pcb->refused_data);
      pcb->refused_data = 0;
    }
    if (pcb->unsent != 0) {
	    MyPrintf_USART1("tcp_pcb_purge: not all data sent\n");
    }
    if (pcb->unacked != 0) {
	    MyPrintf_USART1("tcp_pcb_purge: data left on ->unacked\n");
    }
  ...
  while (pcb != 0) {
    ...
    if (((&pcb->local_ip)->addr == (old_addr)->addr)
      ...
      struct tcp_pcb *next = pcb->next;
	      MyPrintf_USART1("netif_set_ipaddr: aborting TCP pcb %p\n", (void *)pcb);
      tcp_abort(pcb);
      pcb = next;
    } else {
      pcb = pcb->next;
...
tcp_debug_print_pcbs(void)
{
  ...
  MyPrintf_USART1("Active PCB states:\n");
  for (pcb = tcp_active_pcbs; pcb != 0; pcb = pcb->next) {
  ...
  MyPrintf_USART1("Listen PCB states:\n");
  for (pcbl = tcp_listen_pcbs.listen_pcbs; pcbl != 0; pcbl = pcbl->next) {
   
    tcp_debug_print_state(pcbl->state);
  }

  MyPrintf_USART1("TIME-WAIT PCB states:\n");
  for (pcb = tcp_tw_pcbs; pcb != 0; pcb = pcb->next) {
